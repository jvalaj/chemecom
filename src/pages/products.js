  ...
  return (
            ...
            
      {
        data.map((data,i) => (
            ...
            <div className="h-50 sm:80 m-3 border rounded-lg shadow bg-gray-700 border-gray-700 overflow-hidden " key={i}>
                <div className='h-40 sm:h-70 p-5 bg-white flex justify-center items-center overflow-hidden'>
                    <img className=''  layout="fill" objectFit="cover" src={data.imageUrl} alt="" />
                </div>
              <div className="p-5 flex flex-col gap-3">
                    <h5 className=" text-2xl font-bold tracking-tight text-white">{data.name}</h5>
                    <p className=" font-normal text-gray-400">{data.category}</p>
                    <button className="bg-green-400 rounded-md p-2 ml-auto">
                        Rs. {data.price}
                    </button>
              </div>
            </div>
